    ...
    <div className="relative w-full">
      ...
      <div className="relative">
        <InputTypeWithLabel
          ...
          labelClassName={labelClassName}
          inputClassName={`pr-12 [&::-ms-reveal]:hidden [&::-ms-clear]:hidden [&::-webkit-credentials-auto-fill-button]:hidden [&::-webkit-strong-password-auto-fill-button]:hidden ${inputClassName}`}
          {...props} // This passes aria-required, aria-invalid, aria-describedby, disabled, etc.
        />

        {/* Toggle button */}
        <button
          type="button"
          className="absolute inset-y-0 right-3 top-6 flex items-center cursor-pointer text-slate-200 hover:text-slate-100 z-10 disabled:opacity-50 disabled:cursor-not-allowed"
          onClick={() => setShowPassword(!showPassword)}
      ...
      {/* Strength label (always reserved space) */}
      <p className="mt-2 text-xs md:text-sm text-slate-300 min-h-[20px]">
        {value.length > 0 ? (
          <>
            Password strength:{" "}
            <span className={strengthColor}>{strengthLabel}</span>
